...
for grb in t_messages:
    ...
    for i, lat in enumerate(lats):
        for j, x in enumerate(lat):
          radiation_data.append([{"validDate": util.t2s(jst),"analDate": util.t2s(jst2), "lat": util.round_up_to_5_digits(lats[i][j]), "lon": util.round_up_to_5_digits(lons[i][j]), "radiation": util.round_up_to_5_digits(values[i][j])}])

print(radiation_data)

with open("/dist/output.json", 'w') as file:
    # ファイルに書き込む
    file.write(json.dumps(radiation_data))
